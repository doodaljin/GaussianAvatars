...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    perceptual_loss = lps.LPIPS(net="vgg").to("cuda")

    loader_camera_train = DataLoader(scene.getEditCamerasContByTimestep(timestep=828, save_path = dataset.edit_path), batch_size=None, shuffle=True, num_workers=8, pin_memory=True, persistent_workers=True)

    # loader_camera_train = DataLoader(scene.getEditCamerasCont("edit_timesteps.json", dataset.edit_path), batch_size=None, shuffle=True, num_workers=8, pin_memory=True, persistent_workers=True)
        ...
        if iteration % 250 == 1:
            with torch.no_grad():
                try:
                    ...
                    iter_timestep = iter(timesteps)
                    timestep = next(iter_timestep)
                edit_cameras = scene.getEditCamerasContByTimestep(timestep = timestep, save_path = dataset.edit_path)
                loader_camera_train = DataLoader(edit_cameras, batch_size=None, shuffle=False, num_workers=8, pin_memory=True, persistent_workers=False)
                iter_camera_train = iter(loader_camera_train)
