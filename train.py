...
def edit(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from): 
    ...
    len_iter = len(scene.getTrainCameras())
    print("len iter: ", len_iter)
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if debug == 1: 
            save_image(image, os.path.join(debug_path, debug_counter, "render_image.png"))
            save_image(gt_image, os.path.join(debug_path, debug_counter,"gt_image.png"))
            debug = 0
            debug_counter += 1
        if image_edit_flag == 1: 
            ...
            save_edited_path = os.path.join(dataset.edit_path, viewpoint_cam.image_path)
            save_image(edit_image, save_edited_path)
            viewpoint_cam.image_edited_path = save_edited_path
            temp_image = Image.open(viewpoint_cam.image_edited_path)
            im_data = np.array(temp_image.convert("RGBA"))
            norm_data = im_data / 255.0
            arr = norm_data[:,:,:3] * norm_data[:, :, 3:4] + viewpoint_cam.bg * (1 - norm_data[:, :, 3:4])
            temp_image = Image.fromarray(np.array(arr*255.0, dtype=np.byte), "RGB")
            resized_image_rgb = PILtoTorch(temp_image, (viewpoint_cam.image_width, viewpoint_cam.image_height))
            temp_image = resized_image_rgb[:3, ...]
            viewpoint_cam.edited_image = temp_image.clamp(0.0, 1.0)
        else: 
            unedit_counter += 1
        gt_image = viewpoint_cam.edited_image.cuda()
        if unedit_counter % 10000 == 0 and image_edit_flag == 0: 
            image_edit_flag = 1
