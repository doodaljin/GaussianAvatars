...
def edit_dataset(edit_cameras, guidance, prompt_utils, gaussians, pipeline, edit_round, background, save_path):
    ...
    for i in tqdm(range(len(edit_cameras)), desc="Editing progress"):
        ...
        edit_result = guidance(rendering.unsqueeze(0).permute(0, 2, 3, 1), gt_image.unsqueeze(0).permute(0, 2, 3, 1), prompt_utils)
        ...
        save_edited_path = Path(save_path) / split_path[-1]
        # view.image_edited_path = save_edited_path
        edit_cameras.update_edit(i, save_edited_path)
        save_image(edit_image, save_edited_path)
    print("Done editing. \n")
    return edit_cameras

def get_edited_image(view, edit_round, save_path):
    save_edited_path = os.path.normpath(view.image_path)
    split_path = save_edited_path.split(os.sep)
    save_edited_path = Path(save_path) / str(edit_round) / split_path[-1]
    temp_image = Image.open(save_edited_path)
    im_data = np.array(temp_image.convert("RGBA"))
    norm_data = im_data / 255.0
    arr = norm_data[:,:,:3] * norm_data[:, :, 3:4] + view.bg * (1 - norm_data[:, :, 3:4])
    temp_image = Image.fromarray(np.array(arr*255.0, dtype=np.byte), "RGB")
    resized_image_rgb = PILtoTorch(temp_image, (view.image_width, view.image_height))
    temp_image = resized_image_rgb[:3, ...]
    return temp_image.clamp(0.0, 1.0)

def edit(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from): 
    
    guidance = InstructPix2PixGuidance(OmegaConf.create({"min_step_percent": 0.02, "max_step_percent": 0.98}))
    prompt_utils = StableDiffusionPromptProcessor(
        {
            "pretrained_model_name_or_path": "runwayml/stable-diffusion-v1-5",
            "prompt": "Turn him into Hulk",
    ...
    print("len edit: ", len(edit_cameras))
    ...
    edit_round = 0
    debug = 1
    debug_path = "edit_hulk2"
    debug_counter = 0
    perceptual_loss = PerceptualLoss().eval().to(get_device())
    # edit_dataset(loader_camera_train, guidance, "Turn him into an old lady", gaussians, pipe, edit_round, background, dataset.edit_path)
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if debug == 1:
            temp_debug_path = os.path.join(debug_path, str(debug_counter))
            os.makedirs(temp_debug_path, exist_ok = True)
            temp_render_image = image.detach().mul(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).to("cpu", torch.uint8).numpy()
            Image.fromarray(temp_render_image).save(os.path.join(temp_debug_path, "render_image.png"))
            save_image(gt_image, os.path.join(temp_debug_path, "gt_image.png"))
            debug = 0
            debug_counter += 1
